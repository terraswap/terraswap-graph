...
export class ColumbusTerraswap {
    ...
    async getPoolBalance(pair: string, height?: number): Promise<PoolInfoRes | undefined> {
        const data = { pool: {} }
        ...
        return pairs.map(p => ({
            ...
            assets: p.asset_infos.map(
                (a, idx) => ({
                    ...
                    decimals: p.asset_decimals? p.asset_decimals[idx] : 6,
                    amount: "0",
                })
            )
        }))
    ...
    async queryContract(contract: string, data: any, height?: number): Promise<any | undefined> {
